...
- (void)refreshWithTagIsLast:(BOOL)isLast
{
    if (isLast)
    {
        [self addConfirmButtonToPhotoView:self];
    }
    else
    {
        [self removeConfirmButtonInPhotoView:self];
    }
}

- (void)addConfirmButtonToPhotoView:(MJPhotoView*)photoView
{
    if (![photoView viewWithTag:10086])
    {
        UIButton* button_confirm = [UIButton buttonWithType:UIButtonTypeCustom];
        float width = 100;
        float height = 50;
...
- (void)removeConfirmButtonInPhotoView:(MJPhotoView*)photoView
{
    UIView* view_confirm = [photoView viewWithTag:10086];
    [view_confirm removeFromSuperview];
}

- (void)didTapReviewButton:(id)sender
{
    if (self.photoViewDelegate && [self.photoViewDelegate respondsToSelector:@selector(photoViewReviewButtonTapped:)])
    {
        [self.photoViewDelegate photoViewReviewButtonTapped:self];
    }
    [self hide];
}
